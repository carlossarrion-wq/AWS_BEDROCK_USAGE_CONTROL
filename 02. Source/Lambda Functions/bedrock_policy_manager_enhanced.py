...
def send_block_notification(user_id: str, reason: str, usage_record: Dict[str, Any], performed_by: str) -> None:
    """
    Send notification when user is blocked (including enhanced email for admin blocks)
    ...
    try:
        ...
        # Send enhanced email notification for admin blocks
        if email_service and performed_by != 'system':
            try:
                ...
                logger.error(f"Error sending admin blocking email for {user_id}: {str(e)}")
...
def send_unblock_notification(user_id: str, reason: str, performed_by: str) -> None:
    """
    Send notification when user is unblocked (including enhanced email for admin unblocks)
    ...
    try:
        ...
        # Send enhanced email notification for admin unblocks
        if email_service and performed_by != 'system' and performed_by != 'daily_reset':
            try:
                ...
                logger.error(f"Error sending admin unblocking email for {user_id}: {str(e)}")
