#!/usr/bin/env python3
"""
Test Email Notifications for sap_003
====================================

This script tests the email functionality by sending all 3 types of emails
to the sap_003 user using the integrated Gmail email service.
...
class GmailEmailNotificationService:
    ...
    def _send_email(self, to_email: str, subject: str, html_body: str, text_body: str) -> bool:
        ...
        try:
            ...
            print(f"❌ Error sending email to {to_email} via Gmail SMTP: {str(e)}")
            return False
...
def load_gmail_credentials():
    """Load Gmail credentials from environment or prompt user"""
    gmail_user = input("Enter Gmail username: ")
    gmail_password = input("Enter Gmail app password: ")
    return gmail_user, gmail_password

def test_sap_003_emails():
    """Test sending all 3 types of emails to sap_003"""
    print("🧪 Testing Email Notifications for sap_003")
    print("=" * 60)
    
    # Load Gmail credentials
    gmail_user, gmail_password = load_gmail_credentials()
    if not gmail_user or not gmail_password:
        print("❌ Gmail credentials are required")
        return False
    
    try:
        # Initialize Gmail service
        gmail_service = GmailEmailNotificationService(gmail_user, gmail_password)
...
def main():
    """Main function"""
    print("🚀 Email Functionality Test for sap_003")
    print("=" * 60)
    print("📧 Target user: sap_003")
    print("📧 Email address: carlos.sarrion@es.ibm.com (from IAM tags)")
    print("🎨 Features: Spanish language + Professional HTML templates")
    print("=" * 60)
    
    success = test_sap_003_emails()
