        ...
        with connection.cursor() as cursor:
            ...
            return {
                ...
                'body': json.dumps({
                    ...
                    'block_type': 'Manual' if status_result else 'None',
                    'performed_by': 'dashboard_admin' if status_result else None,
...
def execute_admin_blocking(connection, user_id: str, reason: str, performed_by: str, usage_info: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # FIXED: Remove references to non-existent database fields (performed_by, block_type)
            cursor.execute("""
                INSERT INTO user_blocking_status 
                (user_id, is_blocked, blocked_reason, blocked_at, blocked_until, 
                 created_at, updated_at)
                VALUES (%s, 'Y', %s, %s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE
...
def execute_admin_unblocking(connection, user_id: str, reason: str, performed_by: str) -> bool:
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # FIXED: Remove references to non-existent database fields
            cursor.execute("""
                UPDATE user_limits 
